...
at.tweet <- as.character(tweets$text[grep("^\\.?@[a-z0-9_]{1,15}", tolower(tweets$text), perl=T)]);
at.time <- as.character(tweets$time[grep("^\\.?@[a-z0-9_]{1,15}", tolower(tweets$text), perl=T)]);
# at.tweet <- as.character(tweets$text);
...
rt.tweet <- as.character(tweets$text[grep("^rt @[a-z0-9_]{1,15}", tolower(tweets$text), perl=T)]);
rt.time <- as.character(tweets$time[grep("^rt @[a-z0-9_]{1,15}", tolower(tweets$text), perl=T)]);
# rt.tweet <- as.character(tweets$text);
...
at.tweet[at.tweet==""] <- "<NA>";
at.time[at.time==""] <- "<NA>";
rt.sender[rt.sender==""] <- "<NA>";
rt.receiver[rt.receiver==""] <- "<NA>";
rt.tweet[rt.tweet==""] <- "<NA>";
rt.time[rt.time==""] <- "<NA>";

# Create a data frame from the sender-receiver information
ats.df <- data.frame(at.sender, at.receiver,at.tweet, at.time);
rts.df <- data.frame(rt.sender, rt.receiver,rt.tweet, rt.time);
